...
const CARD_CLASS = (isSaas: boolean) =>
  isSaas
    ...
    : 'block rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-indigo-300 hover:shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-offset-2 dark:border-slate-700 dark:bg-slate-900 dark:hover:border-indigo-600 dark:focus-visible:ring-offset-slate-900'

export function DashboardMonthlyDeliveredCard() {
  const { can } = useAuth()
  const isSaas = panelUsesModernShell(usePanelTheme())
  const { from, to, count, paymentsTotal, isLoading, isError } = useDashboardMonthlyDelivered()
    ...
    >
      <div className="flex h-full flex-col justify-between gap-4">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
              {formatMonthNumberLabel(from, to)}
            </p>
          </div>
          <span className="shrink-0 rounded-xl bg-indigo-100 p-2.5 text-indigo-700 dark:bg-indigo-950 dark:text-indigo-300">
        ...
        <div>
          ...
          {amountValue ? (
            <p className="mt-2 text-sm font-semibold tabular-nums text-indigo-700 dark:text-indigo-300">
              {amountValue}{' '}
